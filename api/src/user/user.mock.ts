...
export const NOT_FOUND_IDS = Array.from({ length: 10 }, () => faker.random.uuid());
...
export const userRepositoryMock: () => MockType<Repository<User>> = jest.fn(() => ({
  ...
  findOne: jest.fn((id) => NOT_FOUND_IDS.includes(id) ? null : mockUser({ id })),
...
export const userServiceMock: () => MockType<UserService> = jest.fn(() => ({
  ...
  findOne: jest.fn((id) => NOT_FOUND_IDS.includes(id) ? null : mockUser({ id })),
  create: jest.fn((createUserDto) => mockUser({ ...createUserDto, createdAt: new Date(), updatedAt: new Date() })),
  update: jest.fn((id, updateUserDto) => mockUser({ id, ...updateUserDto, updatedAt: new Date() })),
  remove: jest.fn((id) => NOT_FOUND_IDS.includes(id) ? null : {}),
}));
